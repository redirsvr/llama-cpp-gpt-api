...
func applyModelOptions(p *llama.ModelOptions, embeddings bool) {
    ...
    switch {
    case p.TensorSplit != "" && p.NGPULayers < 0:
        log.Printf("Model options: %+v (TensorSplit задан — fit_params выключен; NGPULayers=%d → все слои на GPU)", p, p.NGPULayers)
    case p.NGPULayers < 0:
        log.Printf("Model options: %+v (NGPULayers=%d → fit_params подберёт слои под VRAM)", p, p.NGPULayers)
    default:
