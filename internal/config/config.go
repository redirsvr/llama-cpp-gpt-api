...
// ModelPreset — индивидуальные опции загрузки конкретной модели.
type ModelPreset struct {
	File                 string                 `yaml:"File"`
	Alias                string                 `yaml:"Alias"`
	ModelOption          map[string]interface{} `yaml:"ModelOption"`
	EmbeddingModelOption map[string]interface{} `yaml:"EmbeddingModelOption"`
}
...
type RAGConfig struct {
	...
	KeywordExtraction        KeywordExtractionConfig `yaml:"KeywordExtraction"`        // entity/keyword extraction (аналог entity descriptions из RAG-Anything)
	QueryRewriting           QueryRewritingConfig    `yaml:"QueryRewriting"`           // улучшение запроса через LLM перед поиском
}

// KeywordExtractionConfig — извлечение ключевых слов/сущностей из чанков (как entity descriptions в RAG-Anything).
type KeywordExtractionConfig struct {
	MaxKeywords int `yaml:"MaxKeywords"` // 0 = отключено (по умолч. 10)
}

// QueryRewritingConfig — улучшение поискового запроса через LLM перед embedding.
type QueryRewritingConfig struct {
	Enabled bool   `yaml:"Enabled"` // использовать LLM для переписывания запроса
	Prompt  string `yaml:"Prompt"`  // кастомный промпт (опционально)
	Model   string `yaml:"Model"`   // модель для переписывания (по умолч. DefaultModel)
}
...
// ModelPresets загружаются из отдельного файла etc/models-preset.yaml.
